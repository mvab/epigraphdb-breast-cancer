...
#htmlwidgets::saveWidget(b, file=paste0( getwd(), "/htmls/exemestane.html"))
```
...
```{r}
ferritin_tidy<-tidy_gwas_to_lit_output(other_gwas)
bc_triples_tidy

# overlap Y -> X

overlap1 <- intersect( ferritin_tidy$term2, bc_triples_tidy$term1)
overlap1_merged<-bind_rows(ferritin_tidy %>% filter(term2 %in% overlap1),
                           bc_triples_tidy %>% filter(term1 %in% overlap1)) %>% 
  select(term1, gs1.localCount, predicate, term2)  %>% 
  filter(gs1.localCount >= 5) %>% 
  group_by(term1, predicate, term2) %>% 
  summarise(count = sum(gs1.localCount)) %>%
  distinct() %>% 
  left_join(predicate_ref %>% select(predicate, direction), by = 'predicate') %>% 
