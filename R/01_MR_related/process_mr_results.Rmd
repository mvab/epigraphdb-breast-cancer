    ...
    if (dim(input_subset_array)[1] != 0) {
      ...
      if (length(effect) == 1){
        ...
        if ('positive' %in% colnames(counts_per_type) & 'negative' %in% colnames(counts_per_type)){
          dir <- 'both'
          tmp <- bind_rows(
                    counts_per_type %>% filter(positive >  `overlaps null`),
                    counts_per_type %>% filter(negative > `overlaps null` )) %>% 
                 distinct()
          ...
          tmp <- counts_per_type %>% filter(positive >  `overlaps null`)
          dir <- 'positive'
          
        } else if ('negative' %in% colnames(counts_per_type)){
          tmp <-  counts_per_type %>% filter(negative > `overlaps null` )
          dir <- 'negative'
        }
...
res_df_all <- bind_rows(lapply(ids, do_MR, 'all')) %>% left_join(exp_help_names)
res_df_pos <- bind_rows(lapply(ids, do_MR, 'ER+')) %>% left_join(exp_help_names)
res_df_neg <- bind_rows(lapply(ids, do_MR, 'ER-')) %>% left_join(exp_help_names)
...
##
redone_MR%>%  select(exposure, id.exposure, exposure_cat) %>% distinct() %>% count(exposure_cat)
...
redone_wide <- redone_MR %>%
  select(id.outcome, id.exposure, exposure_name, effect_direction, method) %>% 
  filter(method %in% c('Inverse variance weighted', 'Wald ratio') )%>% 
...
res_subtypes_wide<- res_subtypes %>% 
  mutate(outcome =   case_when(outcome == 'Luminal B-HER2 negative' ~ 'Luminal B1: ER+PR+HER-',
                               ...
                               TRUE ~ outcome)) %>% 
  select(outcome, id.exposure, exposure_name, effect_direction, method) %>% 
  filter(method %in% c('Inverse variance weighted', 'Wald ratio') )%>% 
  mutate(effect_dir_binary = case_when(effect_direction == 'negative' ~ -1, 
                                       effect_direction == 'positive' ~  1,
                                       effect_direction == 'overlaps null' ~  0)) %>% 
