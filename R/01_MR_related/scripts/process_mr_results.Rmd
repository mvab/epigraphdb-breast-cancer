...
write_tsv(all_mr_validation, "mr_evidence_outputs/all_multiple_testing.tsv")
...
length(unique(x$id.exposure)) # 220
...
length(unique(y$id.exposure)) # 220
...
length(unique(z$id.exposure)) # 175
...
passed_pairs <- all_mr_validation %>%  mutate(mtc = ifelse(qval < 0.05, "*", NA))  %>% select(id.exposure, exposure,outcome, exposure_cat, effect_direction, OR_CI, pval, qval, mtc) %>% distinct() 
length(unique(passed_pairs$id.exposure)) # 103

write_tsv(passed_pairs, "mr_evidence_outputs/passed_multiple_testing.tsv")
...
mtc <- read_tsv("mr_evidence_outputs/all_multiple_testing.tsv")  %>% 
...
# ok find robust stuff

d<- res_w_sens %>% bind_rows %>%
  filter(pval_FDR_adj < 0.054) %>% 
  filter(abs(egger_intercept) < 0.05) %>% 
  filter(exposure_cat %in% c("Proteins"))#, "Metabolites")) 
  
View(d)

d %>% count(exposure, sort=T)


p<-ggplot(d, aes(y=exposure, x=or, label=exposure , colour = outcome)) +
  ...
  #labs(color = "",y = "", x = "beta coefficient (1 SD change in mediator per body size category change)"
       ...
       #)+
  theme(legend.position = "right",  plot.title.position = "plot", axis.title.x=element_text(size=9))
