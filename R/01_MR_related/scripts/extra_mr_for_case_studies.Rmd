...
Testing on all literature intermediates. 
...
mvmr_sens <- full_join(X, all_res, by = c("exposure"="exposure", "outcome"="outcome")) %>% select(test, everything())

total_res_out<- transform(mvmr_sens,group =as.numeric(factor(test)))
total_res_out_dips<- total_res_out %>%
  select(group, everything(), -test) %>% 
  arrange(group) %>% 
  rename("#" = "group", 
