...
case_terms<- read_csv("../02_literature_related/literature_outputs/sankey_terms_storage/lit_terms_IGF1.csv")# %>% filter(value != 'cardiotrophin 1')
...
lit_trait <-lit_df %>% filter(id == main)
...
for (bc_out in outcomes){
  ...
  for (interm in interm_list){
    ...
    x<- bind_rows(
      quick_mr(exp = main, out = interm) %>% select(3:4, OR_CI, effect_direction) %>% mutate(effect = "EM - as med"),# as mediator
      quick_mr(exp = interm, out = main) %>% select(3:4, OR_CI, effect_direction) %>% mutate(effect = "ME - as conf"),# as confounder
      quick_mr(exp = main, out = bc_out) %>% select(3:4, OR_CI, effect_direction) %>% mutate(effect = "EO"),# main to out
      quick_mr(exp = interm, out = bc_out) %>% select(3:4, OR_CI, effect_direction) %>% mutate(effect = "MO"),# interm to out
      quick_mvmr(exp1 = main, exp2 = interm, out = bc_out) %>% select(1:2, OR_CI, effect_direction) %>% mutate(effect = "direct") ) %>%       mutate(test = paste0(interm, " / ", bc_out)) %>% 
      select(test, exposure, everything()) %>% 
      mutate(outcome = gsub("(Combined Oncoarray; iCOGS; GWAS meta analysis)", "", outcome, fixed = T)) 
    
    x <- x %>% mutate(likely_mediator = ifelse(x$effect_direction[1] != 'overlaps null' & x$effect_direction[6] == 'overlaps null',T, F ) )
...
### MR valiadation results for tarits subset from literature intermediates list
...
ft<-width(ft, j = c(2,3,4,6), width=1.5)
ft
```
...
x3 <- mvmr_mixed_sources(id1 = 'prot-b-20',
                   outcome.id = 'ieu-a-1127',
                   id2_tophits_file = "igf_tophits.tsv",
                   id2_gwas_file = "igf_GWAS_tidy_outcome.txt.gz")
x3 %>% kable_it()

x4 <- mvmr_mixed_sources(id1 = 'prot-b-20',
...
x5<- mvmr_mixed_sources(id1 = 'ieu-b-4869',
                   outcome.id = 'ieu-a-1127',
                   id2_tophits_file = "igf_tophits.tsv",
                   id2_gwas_file = "igf_GWAS_tidy_outcome.txt.gz")
...
x6<- mvmr_mixed_sources(id1 = 'finn-b-E4_OBESITY',
                   outcome.id = 'ieu-a-1127',
                   id2_tophits_file = "igf_tophits.tsv",
                   id2_gwas_file = "igf_GWAS_tidy_outcome.txt.gz")
...
x7<- mvmr_mixed_sources(id1 = 'ieu-a-974',
                   outcome.id = 'ieu-a-1127',
                   id2_tophits_file = "igf_tophits.tsv",
                   id2_gwas_file = "igf_GWAS_tidy_outcome.txt.gz")
