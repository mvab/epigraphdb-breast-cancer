...
Traits that have effect on breast cancer:
```{r echo=F}
  ...
  for (interm in interm_list){
    ...
    x<- bind_rows(
      ...
      if (x$exposure[5] == trait_name){
         x <- x %>%
           mutate(likely_mediator = ifelse(x$effect_direction[1] != 'overlaps null' & x$effect_direction[5] == 'overlaps null',T, F ) )
      } else if (x$exposure[6] == trait_name){
        x <- x %>% 
          mutate(likely_mediator = ifelse(x$effect_direction[1] != 'overlaps null' & x$effect_direction[6] == 'overlaps null',T, F ) )
      }
