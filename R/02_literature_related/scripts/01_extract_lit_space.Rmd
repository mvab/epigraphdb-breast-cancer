...
counts_by_cat_df <- 
  ...
  mutate(exposure_cat = factor(exposure_cat, levels= c(
    "Antrophometric",
    ...
    "Lipids",
    "Metabolites",
    "Other biomarkers" ))) %>% arrange(exposure_cat)
...
trait_df_comb <- trait_df_comb %>% 
      mutate(exposure_cat = "combined",
              used_in_lit_analysis = T)
...
lit_counts <- bind_rows(trait_df_all,
...
lid_data<-lid_data  %>% 
  ...
  mutate(exposure_cat = factor(exposure_cat,
                               levels = rev(c("Antrophometric", "Diet and supplements", "Physical activity", "Reproductive", "Sleep", "Drugs", "Alcohol", "Smoking", "Proteins", "Lipids", "Metabolites", "Other biomarkers", "combined"))))

protein_path_data <- read_tsv("../../external_files/pathways/proteins_w_pathways.tsv") 

lid_data <- lid_data %>% left_join(protein_path_data %>% select(gene, id.exposure), by = "id.exposure") %>% mutate(exposure = ifelse(!is.na(gene), gene, exposure))

p<- ggplot(lid_data, aes(y = unique_triples, x= exposure_cat, label= exposure))+
