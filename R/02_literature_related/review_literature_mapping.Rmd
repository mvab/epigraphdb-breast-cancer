...
## data extarcte from v1.2
keep_gwas<-c('ieu-a-1126','finn-a-C3_BREAST', 'finn-a-CD2_BENIGN_BREAST',
             'ukb-a-519', 'ukb-d-C3_BREAST_3', 'ukb-d-D05') # can query with neo4j just for these
dat <- vroom('../../query_results/lit_bc_to_other_litterms_upd.tsv') %>%
      mutate(lit.id = as.character(lit.id)) %>% select(-lit.dp) %>% 
      filter(gwas.id %in% keep_gwas) 

## prod data
dat <- vroom("breast_cancer_litspace_prod.csv") %>%  mutate(lit.id = as.character(lit.id))
...
**Breast cancer space - simple explore **
...
length(unique(bc_triples_tidy$lit.id))# unique papers
...
bc_triples_tidy_count<-bc_triples_tidy_count %>%
