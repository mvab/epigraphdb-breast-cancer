...
dat <- read_tsv("../query_results/bc_all_mr.tsv") %>% 
  ...
  mutate(outcome.details = paste0(outcome.details, "K" )) %>% 
  # tidy up labels
  ...
  mutate(exposure = case_when(grepl("presbyopia", exposure.trait) ~ "Eyesight problems1",
                              grepl("hypermetropia", exposure.trait) ~ "Eyesight problems2",
                              TRUE ~ exposure)) %>% 
...
```{r}
outcome_table<- dat %>% 
  select(outcome, outcome.id, outcome.sample_size, N_case)%>% 
  distinct() %>%
...
plot_heatmap <- function(input){
  ...
  p<-ggplot(input, aes( outcome.details, exposure, fill= mr.b, label = mr.pval, label2 = OR_CI, label3 = exposure.sex)) + 
...
plot_bubble_plot <- function(input){
  ...
  p<-ggplot(input, aes( outcome.details, exposure, 
                  color= mr.b, size = log10pval_trunc, label2 = OR_CI, label3 = exposure.sex)) + 
    geom_point()+
    scale_size(breaks = c(5,7,15))+
