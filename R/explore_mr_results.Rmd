...
all_cats %>% select(exposure_cat, trait) %>% distinct() %>% count(exposure_cat)
...
redone %>%  select(exposure, exposure_cat) %>% distinct() %>% count(exposure_cat)
...
s4 <- list(`All` = redone %>% filter(id.outcome == 'ieu-a-1126') %>% pull(id.exposure),
           `ER+` = redone %>% filter(id.outcome == 'ieu-a-1127') %>% pull(id.exposure),
           `ER-` = redone %>% filter(id.outcome == 'ieu-a-1128') %>% pull(id.exposure))
