...
validated <- read_tsv("01_MR_related/results/mr_evidence_outputs/redone_MRmeds_subsetoutput_ivwV3.tsv") %>% 
  select(id.exp_val = id.exposure , id.out_val = id.outcome, r1.beta_CI_val = beta_CI, r1.pval = pval, r1.nsnp_val=nsnp)
...
traits_bc_validated <- read_tsv("01_MR_related/results/mr_evidence_outputs/redone_MR_subsetoutput_ivwV3.tsv") %>%  # from 03sub
  select(id.trait = id.exposure, id.outcome, OR_CI_val = OR_CI, pval, nsnp_val =nsnp, exposure_cat) 

## 3) joined trait-trait and -bc validation
validated_with_BC <- results_validated %>% 
  left_join(traits_bc_validated, by = c("exposure.id" = "id.trait", "outcome.id" = "id.outcome" )) %>% rename('r2.OR_CI_val' = 'OR_CI_val', "r2.nsnp_val"="nsnp_val", "r2.pval"= "pval") %>% 
  left_join(traits_bc_validated, by = c("med.id" = "id.trait", "outcome.id" = "id.outcome" )) %>% rename('r3.OR_CI_val' = 'OR_CI_val', "r3.nsnp_val"="nsnp_val", "r3.pval"= "pval") %>% 
...
validated_with_BC <- validated_with_BC %>%
  select(type, outcome.id, 
         ...
         r1.beta_CI_val,r1.pval, r1.nsnp_val,  r2.OR_CI_val,r2.pval, r2.nsnp_val, r3.OR_CI_val, r3.pval, r3.nsnp_val)
...
length(unique(validated_with_BC$exposure.id)) # 92
