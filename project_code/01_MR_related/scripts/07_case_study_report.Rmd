...
trait_name = "Haptoglobin"
rmarkdown::render("01_MR_related/scripts/07_case_study_report.Rmd", 
                  params  = list(
                    trait_name = "Haptoglobin",
                    main = "prot-a-1369",
                    main_lit = "prot-a-1369",
                    lit_space_file = "lit_terms_TODO.csv",
                    sens_analysis = F,
                    data_prefix = NA,
                    regenerate_results = F
                  ),
...
med_conf_table <- readxl::read_excel("../results/mr_evidence_outputs/med-table-validatedV3.xlsx", sheet = main) %>% 
     mutate(outcome = case_when(id.outcome =="ieu-a-1126" ~ "Breast cancer",
                             id.outcome =="ieu-a-1127" ~ "ER+ Breast cancer",
                             id.outcome =="ieu-a-1128" ~ "ER- Breast cancer")) %>% 
     mutate(step1.used_instrument = case_when(step1.used_instrument %in% c("cis_and_trans" , "not_protein") ~ "genome-wide",
                                              step1.used_instrument ==  "cis_5e-08_r2=0.01" ~ "cis-region, 5e-08, r2=0.01",
                                              step1.used_instrument == "cis_lessStringent_r2=0.01" ~"cis-region, 1e-05, r2=0.01" )) %>% 
     mutate(step2.used_instrument = case_when(step2.used_instrument %in% c("cis_and_trans" , "not_protein") ~ "genome-wide",
                                              step2.used_instrument ==  "cis_5e-08_r2=0.01" ~ "cis-region, 5e-08, r2=0.01",
                                              step2.used_instrument == "cis_lessStringent_r2=0.01" ~"cis-region, 1e-05, r2=0.01" ))
```
...
if (regenerate_results){
  ...
  # not actually regeneration, but pulling from saved file

  meds_table <- med_conf_table %>% 
    mutate_if(is.numeric, funs(as.character(signif(., 2)))) %>% 
    mutate(step1.beta_CI_pval = paste0(step1.beta_CI, ";\n",step1.pval)) %>% 
    mutate(step2.OR_CI_pval = paste0(step2.OR_CI, ";\n",step2.pval)) %>% 
    ...
    select('exposure', 
           'mediator' ,
           'outcome' ,
           "step1: \n (beta_CI),p.val" = 'step1.beta_CI_pval', 
           "step1: \n p.val.adj" = 'step1.qval', 
           "step1: \n nSNPs" = 'step1.nsnp', 
           "step1: \n SNPs source" =    "step1.used_instrument" ,
           "step2: \n (beta_CI),p.val" = 'step2.OR_CI_pval', 
           "step2: \n p.val.adj" = 'step2.qval',       
           "step2: \n nSNPs" = 'step2.nsnp', 
           "step2: \n SNPs source" =    "step2.used_instrument" ) 


  pairs_todo<- med_conf_table %>%  select(id.exposure, id.med, id.outcome) %>% distinct() 
  
  export1 <- read_tsv("../results/mr_evidence_outputs/redone_MRmeds_fulloutput_fulltable_step1_V3.tsv") %>% 
              filter(id.exposure %in% pairs_todo$id.exposure) %>% 
              filter(id.outcome %in% pairs_todo$id.med) 
  export2 <- read_tsv("../results/mr_evidence_outputs/redone_MRmeds_fulloutput_fulltable_step2_V3.tsv") %>% 
              filter(id.exposure %in% pairs_todo$id.med) %>% 
              filter(id.outcome %in% pairs_todo$id.outcome) 
...
if (all){
  med <- meds_table %>% filter(!grepl("ER", outcome)) %>% pull( mediator) %>% length()
  cat(paste0("**Overall sample**: ", med , " potential mediators from MR-EvE" )) }
```

```{r, echo=F}
if (all){
  ft<-flextable(meds_table %>% filter(!grepl("ER", outcome))) 
 ft<-width(ft, j = c(1,3), width=1.5)
  ft<-width(ft, j = c(2, 4,8), width=3)
...
```{r echo=F, results='asis'}
if (pos){
  med <- meds_table %>% filter(grepl("ER+", outcome))%>% pull( mediator)  %>% unique() %>% length()
  cat(paste0("**ER+ sample**: ", med , " potential mediators from MR-EvE" ))}
```

```{r echo =F}
if (pos) {
  ft<-flextable(meds_table %>% filter(grepl("ER+", outcome, fixed = T))) 
  ft<-width(ft, j = c(1,2,3), width=1.5)
  ft<-width(ft, j = c(4,8), width=3)
  ft<-width(ft, j = c(5,7), width=1.6)
  ft
  }
```
