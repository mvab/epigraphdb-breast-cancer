...
```{r}
#  lit_terms_df_cathD<- case_terms  %>% filter(value == "CATHEPSIN D") %>% 
#      filter(usable ==T) %>% select(value, gwas.id, gwas.name) %>%  drop_na() %>% distinct() %>% 
#      mutate(instruments = NA, used_inst = NA)
#    
#    for (i in 1:length(unique(lit_terms_df_cathD$gwas.id))){ 
#      
#        instruments_selected <-  instrument_selection(lit_terms_df_cathD$gwas.id[i], protein_regions )
#        y <- instruments_selected$SNPs
#        used_inst <- instruments_selected$SNPs_type
#      
#      
#      if (is.null(y)){
#        lit_terms_df_cathD$instruments[i] <- 0
#      } else{
#         
#        if ( NA %in% y$eaf.exposure) {
#          lit_terms_df_cathD$instruments[i] <- 0
#        } else {
#          lit_terms_df_cathD$instruments[i] <- dim(y)[1]
#          lit_terms_df_cathD$used_inst[i] <- used_inst
#          instrument_list[[lit_terms_df_cathD$gwas.id[i]]] <- y
#        }
#      } 
#        print("==============")
#        print(i) 
...
#
#
#  ss_dfD <- ao %>% filter(id %in% c(lit_terms_df_cathD$gwas.id,main)) %>%
#    select(id, sample_size, author, consortium) %>%
#    mutate(sample_size = ifelse(is.na(sample_size) & author == "Neale lab", 361194, sample_size)) %>% 
#    select(exposure.id = id, exposure.sample_size = sample_size)
#  
#cathD_ids <- lit_terms_df_cathD %>% filter(instruments >0) %>% pull(gwas.id)
#  
#  ## do MR for all gwas from the list; for outputs that were set
#res_df_cathD <- bind_rows(lapply(cathD_ids, do_MR, 'ER+', ss_dfD , protein_regions, instrument_list, lit_terms_df_cathD) ) 
...
if (regenerate_results){
  ...
  write_csv(step1mr_both, paste0( "../results/case_study_reports_tidy/intermediate_results_storage/", main, "_F_G_b_resultsV3.csv"))
  ...
  step1mr_both      <- read_csv(paste0( "../results/case_study_reports_tidy/intermediate_results_storage/", main, "_F_G_b_resultsV3.csv"))
}   
...
step1_mr_f <- redone_MR_f %>%  
  filter(method %in% c('Inverse variance weighted', 'Wald ratio')) %>% 
   # add FDR corrected p-val
  arrange(pval) %>% mutate(qval = p.adjust(pval, method = "BH")) %>% 
  select(exposure, id.exposure ,  outcome, id.outcome, beta_CI, pval, p.adj=qval,  nsnp, effect_direction, "egger_intercept", "Q_pval"  , "Fst" ) 
