...
res_subtypes_for_count <- bind_rows(redone_MR,res_subtypes) %>% 
  ...
  mutate(exposure_cat = ifelse(id.exposure =="prot-a-550", "Proteins", exposure_cat)) %>% 
...
all_mr_validation %>% filter(qval < 0.05) %>% pull(id.exposure) %>% unique() %>% length() # 65
...
length(unique(z$id.exposure)) #  102 in BCAC 2017
...
## old outcomes
main_results <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3.tsv")
sens_results <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutput_sensV3.tsv")
...
## also save 
main_results17 <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3.tsv")
...
dat <- all_df %>% 
  select(exposure.id=id.exposure, exposure,  outcome, nsnp, effect_direction, 
...
length(unique(datV3$id.exposure)) # 130
