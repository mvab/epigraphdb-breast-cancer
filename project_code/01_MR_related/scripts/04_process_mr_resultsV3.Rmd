...
redone_MR <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3.tsv") # created in 03
length(unique(redone_MR$id.exposure)) # 208
...
all_mr_validation %>% filter(pval < 0.05) %>% pull(id.exposure) %>% unique() %>% length() # 130
all_mr_validation %>% filter(qval < 0.05) %>% pull(id.exposure) %>% unique() %>% length() # 67
...
length(unique(y$id.exposure)) #  130
...
length(unique(z$id.exposure)) #  105 in BCAC 2020
...
length(unique(w$id.exposure)) #  108 in BCAC 2017
...
passed_pairs %>% filter(mtc == "*") %>% pull(id.exposure) %>% unique() %>% length() #  67
