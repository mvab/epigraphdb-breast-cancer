...
redone_MR <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3cis.tsv") # created in 03
...
res_subtypes <- read_tsv("../results/mr_evidence_outputs/mr_subtypes/all_traits_MR_vs_BCAC2020V3.tsv") %>% 
...
res_subtypes <- res_subtypes %>% 
  ...
  select(-exposure ) %>% 
  rename(id.exposure = exposure.id , exposure = exposure.trait) %>% distinct()
...
```{r}
### this is a table1 in paper making code ---- see table 1 new version with FDR below
### 

all_cats <- read_tsv("../results/mr_evidence_outputs/tidy_traits_by_catV3.tsv")
table1_col1 <-  all_cats %>%  select (exposure.id, exposure_cat) %>% distinct() %>%  count(exposure_cat) #202
...
res_subtypes_for_count <- bind_rows(redone_MR,res_subtypes) %>% 
  ...
  filter(method %in% c('Inverse variance weighted', 'Wald ratio')) %>% 
  # for some reason cat is missing:
  mutate(exposure_cat = ifelse(id.exposure %in% c("prot-a-550", "prot-a-3000" ), "Proteins", exposure_cat)) %>% 
  mutate(exposure_cat = ifelse(id.exposure =="met-c-841", "Metabolites", exposure_cat)) 
  
  

length(unique(res_subtypes$exposure_cat))
unique(res_subtypes_for_count$exposure_cat)
...
counts_by_outcome_df<- left_join(table1_col1, counts_by_outcome_df) %>% column_to_rownames('exposure_cat')  %>% replace(is.na(.), 0) 
...
write_tsv(table1, "../results/mr_evidence_outputs/mr_subtypes/table1_counts_by_exposure_catV3.tsv")
```


```{r}
# table 1 mini version with FDR: 
res_subtypes_for_count_mtc <- left_join(res_subtypes_for_count, mtc)  # mtc is created later in the code

counts_by_outcome <- list(
  total = res_subtypes_for_count_mtc %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  totalFDR = res_subtypes_for_count_mtc %>% filter(pval_FDR_adj < 0.05) %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  total2017 = res_subtypes_for_count_mtc %>% filter(grepl("Combined", outcome)) %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total2017=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  total2017FDR = res_subtypes_for_count_mtc %>% filter(pval_FDR_adj < 0.05)%>% filter(grepl("Combined", outcome)) %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total2017=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),

  total2020 = res_subtypes_for_count_mtc %>% filter(!grepl("Combined", outcome)) %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total2020=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  total2020FDR = res_subtypes_for_count_mtc %>% filter(pval_FDR_adj < 0.05)%>% filter(!grepl("Combined", outcome)) %>% select(id.exposure, exposure_cat) %>% distinct() %>%  count(exposure_cat) %>% rename(total2020=n) %>% 
    column_to_rownames('exposure_cat') %>% t() %>% as_data_frame()
  )
...
table1FDR <- counts_by_outcome_df %>% 
    rownames_to_column('exposure_cat') %>% 
    mutate(exposure_cat = case_when(exposure_cat == "...13" ~"TOTAL", 
                                    exposure_cat == 'Antrophometric' ~ 'Anthropometric',
                                     exposure_cat == 'Drugs' ~ 'Medication',
...
write_csv(table1FDR, "../results/mr_evidence_outputs/mr_subtypes/table1_counts_by_exposure_catV3_FDR.csv")
...
```{r}
# legacy
#library(flextable) #### need to get this to work for V3
#library(officer)
...
merged <- left_join(redone_wide, 
          ...
          left_join(all_cats %>% select('exposure_name'='exposure', 'id.exposure'='exposure.id', 'exposure_cat')) %>% 
          mutate(label = paste0("(", id.exposure, ") ", exposure_name )) %>% 
          select(exposure_cat, label, exposure_name, everything()) %>% 
          distinct()


write_tsv(merged, "../results/mr_evidence_outputs/trait_manual_ivw_subtypes_mergedV3.tsv")

proteins <- merged %>% filter(exposure_cat == "Proteins") %>% select(id.exposure, exposure)
dim(proteins) # 108 + 2 I dropped because their SNPs are not in outcomes??
write_tsv(proteins, "../results/mr_evidence_outputs/protein_in_final_setV3.tsv")
```
...
res_subtypes <- read_tsv("../results/mr_evidence_outputs/mr_subtypes/all_traits_MR_vs_BCAC2020V3.tsv") %>% filter(outcome != "TNBC_BRCA1 ER-PR-HER-")
## category update 
res_subtypes <- res_subtypes %>% 
  rename(exposure.trait=exposure, exposure.id = id.exposure ) %>%
  create_exposure_categories() %>%  filter(exposure_cat != 'other') %>% 
...
all_mr_validation %>% filter(pval < 0.05) %>% pull(id.exposure) %>% unique() %>% length() # 129 
all_mr_validation %>% filter(qval < 0.05) %>% pull(id.exposure) %>% unique() %>% length() # 63

write_tsv(all_mr_validation, "../results/mr_evidence_outputs/all_multiple_testingV3.tsv")
...
length(unique(x$id.exposure)) #  129
...
length(unique(y$id.exposure)) #  129
...
length(unique(z$id.exposure)) #  101 in BCAC 2017
...
length(unique(w$id.exposure)) #  109 in BCAC 2020
...
passed_pairs %>% filter(mtc == "*") %>% pull(id.exposure) %>% unique() %>% length() #  65
write_tsv(passed_pairs, "../results/mr_evidence_outputs/passed_multiple_testingV3.tsv")
...
mtc <- read_tsv("../results/mr_evidence_outputs/all_multiple_testingV3.tsv")  %>% 
  select(id.exposure, outcome,qval )%>% rename(pval_FDR_adj = qval)

## old outcomes
main_results <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3cis.tsv") %>% 
    mutate(exposure_cat = ifelse(id.exposure %in% c("prot-a-550", "prot-a-3000" ), "Proteins", exposure_cat)) %>% 
    mutate(exposure_cat = ifelse(id.exposure =="met-c-841", "Metabolites", exposure_cat)) 
sens_results <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutput_sensV3cis.tsv")
...
main_results <- read_tsv("../results/mr_evidence_outputs/mr_subtypes/all_traits_MR_vs_BCAC2020V3.tsv") %>%  mutate(id.outcome = outcome) %>% 
    mutate(exposure_cat = ifelse(id.exposure %in% c("prot-a-550", "prot-a-3000" ), "Proteins", exposure_cat)) %>% 
    mutate(exposure_cat = ifelse(id.exposure =="met-c-841", "Metabolites", exposure_cat)) 
sens_results <- read_tsv("../results/mr_evidence_outputs/mr_subtypes/all_traits_sensMR_vs_BCAC2020V3.tsv") %>%  mutate(id.outcome = outcome)
...
writexl::write_xlsx(res_w_sens, "../results/mr_evidence_outputs/all_data_with_sens_filtersV3.xlsx")

## also save 
main_results17 <- read_tsv("../results/mr_evidence_outputs/redone_MR_fulloutputV3cis.tsv")
main_results20 <- read_tsv("../results/mr_evidence_outputs/mr_subtypes/all_traits_MR_vs_BCAC2020V3.tsv") %>% 
                              mutate(id.outcome = outcome) %>% filter(outcome != "TNBC_BRCA1 ER-PR-HER-")

all_mr_tests <- bind_rows(main_results17,main_results20 ) %>% 
                arrange(exposure_cat, id.exposure) %>% 
                select(-exposure_name)

write_tsv(all_mr_tests, "../results/mr_evidence_outputs/all_data_validationV3.tsv")
all_mr_tests <- read_tsv("../results/mr_evidence_outputs/all_data_validationV3.tsv") # supl data 3
length(unique(all_mr_tests$id.exposure)) # 202
```
...
```{r}
# testing subsetting by sensitivity
library(rio)
res_w_sens <- import_list("../results/mr_evidence_outputs/all_data_with_sens_filtersV3.xlsx") # supl data 2


all_df <- bind_rows(res_w_sens)

dat <- all_df %>% 
  select(exposure.id=id.exposure, exposure,  outcome, nsnp, effect_direction, used_instrument,exposure_cat, method,
         egger_intercept, egger_intercept_less_than_0.05, heterogeneity_Q_pval, heterogeneity_Q_pval_less_than_0.05, Fst) %>% 
  mutate(outcome = case_when(outcome =="Breast cancer BCAC 2020" ~ "BCAC'20",
                             outcome =="Breast cancer (Combined Oncoarray; iCOGS; GWAS meta analysis)" ~ "BCAC'17",
  ...
  mutate(hazards = case_when(egger_intercept_less_than_0.05 == T & heterogeneity_Q_pval_less_than_0.05 == F ~ "",
                                 egger_intercept_less_than_0.05 == T & heterogeneity_Q_pval_less_than_0.05 != F ~ "H",
                                 egger_intercept_less_than_0.05 != T & heterogeneity_Q_pval_less_than_0.05 == F ~ "P",
                                 egger_intercept_less_than_0.05 != T & heterogeneity_Q_pval_less_than_0.05 != F ~ "HP",
                                 nsnp <=2 ~ "X",
                                 TRUE ~ NA)) 
 
dat %>% count(hazards)
dat %>% filter(exposure_cat=="Proteins") %>% count(used_instrument) %>% View()
dat %>% select(exposure.id, used_instrument) %>% distinct() %>% count(exposure.id) %>% View() 
# prot-a-2356 and  prot-b-55 are genome-wide in BCAC 2020 but cis in 2017; going to put genome-side for all

dat <- dat %>% 
  mutate(used_instrument = ifelse(exposure.id %in% c('prot-a-2356', 'prot-b-55'), "genome-wide",used_instrument )) %>% 
  mutate(used_inst_code = ifelse(exposure_cat=="Proteins" & used_instrument == "genome-wide", "", "*"))

write_tsv(dat, "../results/mr_evidence_outputs/all_data_with_sens_filters_hazardsV3.tsv")
```

```{r}

dat <- read_tsv("../results/mr_evidence_outputs/all_data_with_sens_filters_hazardsV3.tsv") # supl data 2




library(rio)
res_w_sens <- import_list("../results/mr_evidence_outputs/all_data_with_sens_filtersV3.xlsx")
...
# compare with previous final list
```{r}
datV3 <- read_tsv("../results/mr_evidence_outputs/all_multiple_testingV3.tsv") %>% filter(pval < 0.05)
length(unique(datV3$id.exposure)) # 129

datV2 <- read_tsv("../results/mr_evidence_outputs/all_multiple_testing.tsv") %>% filter(pval < 0.05)
length(unique(datV2$id.exposure)) # 213



new <- setdiff(datV3$id.exposure, datV2$id.exposure)
length(new)# 26
new_df<-datV3 %>%  filter(id.exposure %in% new) %>% select(id.exposure, exposure, exposure_cat) %>% distinct()
write_tsv(new_df, "../results/mr_evidence_outputs/comparisonsV2vsV3_new_traits.tsv")

missing <- setdiff(datV2$id.exposure, datV3$id.exposure)
length(missing) # 110
missing_df <- datV2 %>%  filter(id.exposure %in% missing) %>% select(id.exposure, exposure, exposure_cat) %>% distinct() 
write_tsv(missing_df, "../results/mr_evidence_outputs/comparisonsV2vsV3_missing_traits.tsv")
```
