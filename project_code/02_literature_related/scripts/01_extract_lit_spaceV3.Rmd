...
trait_df_all <- read_tsv("../../01_MR_related/results/mr_evidence_outputs/trait_manual_ivw_subtypes_mergedV3_copy.tsv") %>% 
  ...
  select(id.exposure, exposure, exposure_cat) %>% distinct() # 129


dup_name <- trait_df_all %>% count(exposure) %>% filter(n>1) %>% pull(exposure)
trait_df_all %>% filter(exposure %in% dup_name) %>% View()


# traits with the same name or trait that ar eunlike to match to something meaningful
ignore_traits<- c('ieu-a-95', 'ieu-a-974', 'ukb-a-248',  'ukb-a-34', 'ukb-a-35', 'ukb-b-16881', 'ukb-a-317',
                  'ieu-a-299', 	'ukb-a-388', 'ieu-a-101','ieu-a-51',  'prot-a-1541','ieu-a-62', 	'ukb-b-9405', 'ieu-a-105', 'ieu-a-109', 'ieu-a-783',
                  "ieu-a-89", "ieu-a-99", "ieu-a-49", "ukb-b-12039")
trait_df_all %>% filter(id.exposure %in% ignore_traits) %>% View()

lipids_to_save <- c(#'met-c-932', 'met-c-912', 'met-c-891', 'met-c-928', 'met-c-913',# vldl
                    'met-c-923', 'met-c-877', 'met-c-898', 'met-c-900',	'met-c-878', "met-c-864", # hdl
                    'ieu-a-780' , "ukb-d-30760_irnt") # hdl
...
trait_df_all %>% count(lit_analysis) # True 107
write_tsv(trait_df_all, "../results/literature_outputs/traits_marked_for_lit_analysisV3.tsv")
...
save(lit_spaces, file="../results/literature_outputs/lit_spaces_finalsetV3.RData")
```
...
#load("../results/literature_outputs/lit_spaces_finalsetV3.RData")
...
save(tidy_litspace, file="../results/literature_outputs/lit_spaces_finalset_tidyV3.RData")
```
...
# adhoc fix
trait_df_all<-trait_df_all %>% mutate(exposure_cat = ifelse(id.exposure =="prot-a-550", "Proteins", exposure_cat))

# replace NA to 0
trait_df_all <- trait_df_all %>%  mutate(across(5:8, ~replace_na(.x, 0)))

write_tsv(trait_df_all,
          "../results/literature_outputs/traits_marked_for_lit_analysis_with_sizeV3.tsv")

trait_df_all <- read_tsv("../results/literature_outputs/traits_marked_for_lit_analysis_with_sizeV3.tsv")
...
tidy_litspace[["ukb-d-30760_irnt"]] %>% View()
...
#  `<=5`=trait_df_all %>% filter(lit_space_size_tidy > 0 & lit_space_size_tidy <=5) %>% count(exposure_cat) %>% rename(`<=5`=n) %>% column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  `empty or <50`=trait_df_all %>% filter(used_in_lit_analysis ==1 & lit_space_size_tidy <50) %>% count(exposure_cat) %>% rename(`empty or <50`=n) %>% column_to_rownames('exposure_cat') %>% t() %>% as_data_frame(),
  `>=50`=trait_df_all %>% filter( used_in_lit_analysis ==1 & lit_space_size_tidy >= 50) %>% count(exposure_cat) %>% rename(`>=50`=n) %>% column_to_rownames('exposure_cat') %>% t() %>% as_data_frame())

counts_by_cat_df <- 
  bind_rows(counts_by_cat) %>% t() %>% as.data.frame() %>% 
   mutate(across(V1:V3, ~replace_na(.x, 0)))
...
counts_by_cat_df<- counts_by_cat_df %>%
  ...
  mutate(exposure_cat = ifelse(exposure_cat == "...12", "TOTAL", exposure_cat)) %>% 
...
colnames(counts_by_cat_df)[2:4] <- names(counts_by_cat)
write_csv(counts_by_cat_df, "../results/literature_outputs/available_litspace_counts_by_exposure_catV3.csv")
```
...
related_traits <- list(
 # height = c('ieu-a-1034','ieu-a-89'),
  bmi = c("ieu-a-835", "	ieu-a-107"),
  obesity = c( 'ieu-a-90',	   'ieu-a-91',	'ieu-a-85'),
 # waist_hip = c('ieu-a-68','ieu-a-75','ieu-a-99'),
  #fruit = c('ukb-b-3881','ukb-b-16576','ukb-b-2221'),
  menopause = c('ukb-a-316' , 'ukb-b-17422'),
 # hrt = c('ukb-b-9487', 'ukb-b-18541'),
  ...
  hdl = c( 'ukb-d-30760_irnt', 'met-c-864', 'met-c-923')
...
save(combined_lit_spaces, file="../results/literature_outputs/lit_spaces_combined_traitsV3.RData")
```

```{r}
## tidying lit sapces for loop
load("../results/literature_outputs/lit_spaces_combined_traitsV3.RData")
...
save(tidy_combined_litspace, file="../results/literature_outputs/lit_spaces_combined_traits_tidyV3.RData")
...
load("../results/literature_outputs/lit_spaces_combined_traits_tidyV3.RData")
...
for (i in 1:length(trait_df_comb$trait)){
  ...
  trait_df_comb$lit_space_size[i] <- dim(combined_lit_spaces[[trait_comb_name]])[1]
  
  trait_df_comb$lit_space_size_tidy[i] <- dim(tidy_combined_litspace[[trait_comb_name]] %>% select(term1, predicate, term2) %>% distinct() )[1]
...
trait_df_comb$exposure_cat <- c("Anthropometric","Anthropometric","Reproductive","Lipids")
trait_df_comb <- trait_df_comb %>% mutate(trait = paste0("combined_", trait)) %>% 
                                   rename(exposure = trait) %>% 
                                   mutate(id.exposure = exposure)

  
write_tsv(trait_df_comb, "../results/literature_outputs/traits_marked_for_lit_analysis_combinedV3.tsv")


# make a jopint lit_counts df
lit_counts <- bind_rows(trait_df_all %>% mutate(combined=F) ,
                        trait_df_comb %>% mutate(combined=T))
  #filter(!id.exposure %in% unlist(related_traits))

write_tsv(lit_counts, "../results/literature_outputs/lit_space_stats_V3.tsv") # supl data 2 thesis?

```

```{r}
# make fiel with all tisy lit spaces for exposting to supplementary
load("../results/literature_outputs/lit_spaces_finalset_tidyV3.RData")
length(tidy_litspace)
load("../results/literature_outputs/lit_spaces_combined_traits_tidyV3.RData")
length(tidy_combined_litspace)

lit_space_stats<- read_tsv("../results/literature_outputs/lit_space_stats_V3.tsv")
...
sheet_list<- tibble(id.exposure=names(all_tidy_spaces)) %>% 
             full_join(lit_space_stats) %>% 
             mutate(exposure = ifelse(is.na(exposure), "combined literature space", exposure)) %>% 
             mutate(priority = case_when(id.exposure %in% c('ukb-d-30760_irnt', 'ieu-a-1096', 'ukb-b-4650') ~ 1,
                                         grepl("combined", id.exposure) ~ 3,
                                         used_in_lit_analysis == F ~ 4,
                                         TRUE ~ 2)) %>% 
            arrange(priority) %>% select(-priority) %>% 
            rename(queried =used_in_lit_analysis) %>% 
            mutate(queried = ifelse(grepl("combined", id.exposure), "combined", queried)) %>% 
            mutate(sheet = row_number()+1) %>% select(sheet, everything()) %>% 
            filter(id.exposure != 'navigation')

sheet_list_list<-list(sheet_list)
names(sheet_list_list) <- "navigation"

# todo: manually add link_to_sheet column: =HYPERLINK("#'"&B2&"'!A2",B2)

# rearrange items in lst:
tidy_trait_list <- sheet_list %>% filter(queried %in% c("TRUE"), lit_space_size > 0) %>% pull(id.exposure)
tidy_litspace2 <- tidy_litspace[ tidy_trait_list ]

all_tidy_spaces<-c(sheet_list_list, tidy_litspace2, tidy_combined_litspace)



writexl::write_xlsx(all_tidy_spaces, "../results/literature_outputs/all_tidy_spacesV3.xlsx") # supl data 3 thesis
